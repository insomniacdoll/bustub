...
auto BufferPoolManager::NewPage() -> page_id_t {
  ...
  // Atomically fetch and increment the next page ID
...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool {
  ...
  if (it == page_table_.end()) {
    // Page is not in memory, just deallocate on disk
    disk_scheduler_->DeallocatePage(page_id);
    return true;
  }
  
  // Page is in memory, check if it's pinned
...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  ...
  if (it != page_table_.end()) {
    ...
    // Record access to update the replacer (this will add frame to replacer if not already there)
    replacer_->RecordAccess(frame_id, page_id, access_type);
    // Mark as not evictable while we're accessing it
    replacer_->SetEvictable(frame_id, false);
    
    // Release latch before acquiring frame lock to avoid deadlock
    latch.unlock();
  ...
  // Page is not in memory, need to find a frame
  if (!free_frames_.empty()) {
    ...
    // Get the evicted frame
    auto &evicted_frame_header = frames_[frame_id];
    
    // Find the page ID of the evicted page
    ...
    if (evicted_frame_header->is_dirty_) {
      auto promise = disk_scheduler_->CreatePromise();
      auto future = promise.get_future();
      
      DiskRequest request{
        .is_write_ = true,
        .data_ = evicted_frame_header->GetDataMut(),
        .page_id_ = evicted_page_id,
      ...
      // Release latch while waiting for I/O
      latch.unlock();
      future.get();
      latch.lock();
    }
  ...
  // Record access to add frame to replacer
  replacer_->RecordAccess(frame_id, page_id, access_type);
  // Mark as not evictable while we're accessing it
  replacer_->SetEvictable(frame_id, false);
  
  // Release latch before acquiring frame lock and waiting for I/O
  latch.unlock();
  future.get();
...
auto BufferPoolManager::FlushPageUnsafe(page_id_t page_id) -> bool {
  ...
  if (frame->is_dirty_) {
    ...
    // Note: We don't reset the dirty flag here since this is "unsafe"
    // The dirty flag should only be reset by the page guard after a successful flush
  }
...
auto BufferPoolManager::FlushPage(page_id_t page_id) -> bool {
  ...
  // Acquire exclusive lock on the frame to ensure consistent state
  frame->rwlatch_.lock();
...
void BufferPoolManager::FlushAllPages() {
  ...
  // Collect all frame IDs first to avoid modifying the page table while iterating
  std::vector<std::pair<page_id_t, frame_id_t>> page_list;
  page_list.reserve(page_table_.size());
  for (const auto &[page_id, frame_id] : page_table_) {
    page_list.push_back({page_id, frame_id});
  }
  
  // Iterate through all pages and flush them
  for (const auto &[page_id, frame_id] : page_list) {
    auto &frame = frames_[frame_id];
